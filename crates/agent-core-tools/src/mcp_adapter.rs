...
impl Tool for McpToolAdapter {
    ...
    async fn execute(
        ...
        result.call_id = call_id.0.clone();
...
mod tests {
    ...
    use agent_core_types::{
        MessagePart, ToolCallId, ToolOrigin, ToolOutputMode, ToolResult, ToolSpec,
    };
    use serde_json::json;
    use std::sync::Arc;
    ...
    async fn mcp_adapter_normalizes_call_and_tool_identity() {
        ...
        assert_eq!(result.call_id, "local-call-1");
        ...
        let adapter = McpToolAdapter::new(
            ...
            Arc::new(|_call_id, _arguments| {
                Box::pin(async move {
                    Ok(ToolResult {
                        id: ToolCallId::new(),
                        call_id: "remote-call-2".to_string(),
