...
fn origin_matches(matcher: &ToolOriginMatcher, origin: &agent_core_types::ToolOrigin) -> bool {
    match (matcher, origin) {
        (ToolOriginMatcher::Local, agent_core_types::ToolOrigin::Local) => true,
        (
            ToolOriginMatcher::McpServer { server_name: left },
            agent_core_types::ToolOrigin::Mcp { server_name: right },
        ) => left == right,
        (
            ToolOriginMatcher::Provider { provider: left },
            agent_core_types::ToolOrigin::Provider { provider: right },
        ) => left == right,
        _ => false,
    }
}
...
mod tests {
    ...
    use agent_core_types::{ToolCall, ToolCallId, ToolOrigin, ToolOutputMode, ToolSpec};
    use serde_json::json;
