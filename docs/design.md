...
- `web_search` now goes through a provider boundary instead of hardcoding the Bing RSS bootstrap path as the contract. The request surface carries locale, freshness, and source mode explicitly, backend metadata makes fallback capability gaps visible, search results expose stable citation/source ids instead of only transcript prose, and local freshness filtering reports when it only ran in best-effort mode.
