...
- Search engines now live behind a small registry under `web/search/engines/`. The tool entry normalizes requests, applies policy/domain filtering, and shapes the unified output contract, while engine modules own protocol-specific request building, pagination, and response parsing. The bundled set is `bing_rss` for lightweight fallback plus `brave_api` for a hosted JSON backend, and additional engines such as DuckDuckGo or Exa can be added without reopening the tool entrypoint.
...
- Feature-enabled `web_search` is intentionally mixed-mode today. The registry can route to a richer hosted backend (`brave_api`) when configured, but the default fallback remains `bing_rss`, which still does not provide hosted-tool quality ranking, citations, or native freshness/source-mode enforcement.
