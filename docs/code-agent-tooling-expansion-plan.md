...
| Phase 4: Checkpoint And Restore | Complete | `checkpoint_list`, `checkpoint_summarize`, and `checkpoint_restore` now exist, `write` / `edit` / `patch_files` persist durable pre-mutation checkpoints, and both the rollback overlay and the model-visible restore tool now share transcript-only, code-only, and combined code+conversation restore semantics. |
...
| Cross-cutting: Tool Review Surface | Complete | `ToolReview` is now a typed item-based review substrate instead of a diff-only file list, the TUI can open the same centered review overlay for running, failed, and completed tools with structured sections or file diffs, and `review_start` now lets the model replay recent persisted tool activity back through that shared review surface. |
