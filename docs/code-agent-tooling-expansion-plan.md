...
| Phase 8: Browser / Computer Use | In Progress | Feature-gated `browser_open` now creates typed browser session records and persists browser lifecycle events separately from shell monitors; DOM interaction, screenshots, and close/update flows are still outstanding. |
