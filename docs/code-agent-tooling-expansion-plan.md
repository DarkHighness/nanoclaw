...
| Phase 6: Cron / Automation | In Progress | `cron_create` and `cron_list` now exist behind `automation-tools`; schedules are still session-local, and `cron_delete` plus restart persistence remain outstanding. |
...
- `cron_create` and `cron_list` are implemented behind the `automation-tools` feature.
...
- `cron_delete` and durable restart recovery remain outstanding.
