...
| Phase 4: Checkpoint And Restore | In Progress | `checkpoint_list` and `checkpoint_restore` now exist, `write` / `edit` / `patch_files` persist durable pre-mutation checkpoints, the rollback overlay can now switch between transcript-only rewind and combined code+conversation restore for checkpointed turns, and `checkpoint_summarize` is the main remaining gap. |
