...
| Phase 4: Checkpoint And Restore | In Progress | `checkpoint_list` and `checkpoint_restore` now exist, `write` / `edit` / `patch_files` persist durable pre-mutation checkpoints, and the TUI rollback copy now explicitly distinguishes transcript rewind from code restore; transcript-plus-code restore still needs a dedicated operator flow. |
