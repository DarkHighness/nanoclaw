...
- Search engines now live behind a small registry under `web/search/engines/`. The tool entry normalizes requests, applies policy/domain filtering, and shapes the unified output contract, while engine modules own protocol-specific request building, pagination, and response parsing. The bundled set now spans fallback engines (`bing_rss`, `duckduckgo_html`) and hosted JSON APIs (`brave_api`, `exa_api`), so additional providers can be added without reopening the tool entrypoint.
- `web_search` now exposes backend choice as part of the tool contract instead of only host config. Calls can request a specific engine, or let the registry auto-select the richest configured backend. Structured output records both the requested selector and the resolved engine so hosted-vs-fallback behavior stays auditable in transcripts.
- `web_search_backends` exposes the same registry as a read-only inspection tool. Hosts and agents can enumerate configured and unconfigured engines, selector aliases, capability coverage, missing requirements, and the current auto-selection result before issuing a networked search.
...
- Feature-enabled `web_search` is intentionally mixed-mode today. Auto-selection prefers richer hosted backends (`exa_api`, then `brave_api`) when configured, falls back to the more stable feed-backed `bing_rss`, and keeps HTML-scrape engines such as `duckduckgo_html` as the last resort because they can still be interrupted by bot challenges. Those fallback paths do not provide hosted-tool quality ranking, citations, or native freshness/source-mode enforcement.
