...
usage() {
  cat <<'EOF'
Usage:
  collect_pressure_snapshot.sh --output DIR [--driver host|daemon]
                               [--pid PID[,PID...]] [--uid UID] [--gid GID] [--cgroup PATH]
                               [--sched-claw-bin PATH] [--daemon-socket PATH] [--overwrite]

Examples:
  collect_pressure_snapshot.sh --output artifacts/pressure --pid 4242
  collect_pressure_snapshot.sh --driver daemon --output artifacts/pressure-root --cgroup work.slice
EOF
}
...
capture_cgroup_context() {
  ...
  mkdir -p "$out_dir"
  snapshot_file "$cgroup_path/cpu.pressure" "$out_dir/cpu.pressure"
  snapshot_file "$cgroup_path/io.pressure" "$out_dir/io.pressure"
  snapshot_file "$cgroup_path/memory.pressure" "$out_dir/memory.pressure"
...
if [[ "$driver" == "daemon" ]]; then
  ...
  daemon_args+=(daemon collect-pressure --output-dir "$output_dir" --style plain)
...
snapshot_file "/proc/pressure/cpu" "$output_dir/proc.pressure.cpu"
snapshot_file "/proc/pressure/io" "$output_dir/proc.pressure.io"
snapshot_file "/proc/pressure/memory" "$output_dir/proc.pressure.memory"
