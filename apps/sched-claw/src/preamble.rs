...
pub fn build_system_preamble(
    ...
    let mut preamble = vec![
        ...
        "When an iteration is mostly a template-backed knob or small policy mutation, prefer `experiment mutate-candidate` over hand-assembling fork, materialize, and set-candidate steps."
            .to_string(),
        "Use the experiment evaluation policy to keep promotion criteria explicit: minimum run counts, minimum primary improvement, optional improving-run ratio, optional MAD outlier gates, and optional primary-metric spread gates should live in the manifest instead of ad hoc transcript rules."
            .to_string(),
