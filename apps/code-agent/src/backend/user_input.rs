...
pub(crate) struct UserInputPrompt {
...
pub(crate) struct UserInputCoordinator {
    inner: Arc<RwLock<UserInputCoordinatorState>>,
}

impl UserInputCoordinator {
    pub(crate) fn snapshot(&self) -> Option<UserInputPrompt> {
        self.inner.read().unwrap().prompt.clone()
    }

    pub(crate) fn resolve(&self, response: UserInputResponse) -> bool {
...
pub(crate) struct SessionUserInputHandler {
    coordinator: UserInputCoordinator,
}

impl SessionUserInputHandler {
    pub(crate) fn new(coordinator: UserInputCoordinator) -> Self {
        Self { coordinator }
    }
}
...
pub(crate) struct NonInteractiveUserInputHandler {
    reason: String,
}
