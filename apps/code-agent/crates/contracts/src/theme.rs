...
impl ThemeCatalog {
    ...
    pub fn palette_for(&self, theme_id: &str) -> Option<ThemePalette> {
        self.themes
            .iter()
...
pub fn install_theme_catalog(catalog: ThemeCatalog) {
    let active_theme = catalog.active_theme.clone();
    *active_theme_lock()
        .write()
...
pub fn theme_summaries() -> Vec<ThemeSummary> {
    active_theme_lock()
        .read()
...
mod tests {
    ...
    fn parses_builtin_theme_catalog() {
        ...
        assert!(catalog.contains("linen"));
        assert!(catalog.contains("meadow"));
        assert!(catalog.contains("paper"));
        assert!(catalog.contains("radar"));
        assert!(catalog.contains("rosewood"));
        assert!(catalog.contains("seabreeze"));
        assert!(catalog.contains("signal"));
        assert!(catalog.contains("spruce"));
        assert!(catalog.contains("studio"));
        assert!(catalog.contains("terracotta"));
    }
...
header = "#17120d"
        ...
        let catalog = load_theme_catalog(dir.path(), Some("themes.toml"), None).unwrap();
        assert_eq!(catalog.active_theme, "codex");
        assert!(catalog.contains("paper"));
        assert!(catalog.contains("graphite"));
    }
