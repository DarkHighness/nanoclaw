...
use agent::types::{AgentSessionId, Message, SessionEventEnvelope};
use anyhow::Result;

pub use agent::runtime::HISTORY_ONLY_RESUME_REASON;

pub fn can_resume_agent_session(
...
pub fn visible_transcript(session: &RuntimeSession) -> Vec<Message> {
    let visible_indices = if let Some(summary_index) = session.compaction_summary_index {
        let mut indices =
            Vec::with_capacity(1 + session.retained_tail_indices.len() + session.transcript.len());
...
#[cfg(test)]
mod tests {
    use super::{HISTORY_ONLY_RESUME_REASON, reconstruct_runtime_session, visible_transcript};
    use agent::types::{
    ...
    #[test]
    fn visible_transcript_projects_compacted_window_in_visible_order() {
        let session_id = SessionId::from("session_demo");
        let agent_session_id = AgentSessionId::from("agent_demo");
        let first = Message::user("draft one").with_message_id(MessageId::from("msg_one"));
        let second = Message::assistant("draft two").with_message_id(MessageId::from("msg_two"));
        ...
        let reconstructed = reconstruct_runtime_session(&events, &agent_session_id).unwrap();
        let visible = visible_transcript(&reconstructed);
