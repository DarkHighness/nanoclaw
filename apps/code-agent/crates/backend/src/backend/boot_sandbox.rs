...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SandboxFallbackNotice {
...
pub fn inject_process_env(env_map: &EnvMap) {
    // This runs before the Tokio runtime starts, so mutating process env is safe here.
    env_map.apply_to_process();
}

pub fn build_tool_context(workspace_root: &Path, options: &AppOptions) -> ToolExecutionContext {
...
pub fn tool_context_for_profile(
...
pub fn log_sandbox_status(status: &SandboxBackendStatus) {
    match status {
...
pub fn inspect_sandbox_preflight(workspace_root: &Path, options: &AppOptions) -> SandboxPreflight {
...
pub fn build_sandbox_fallback_notice(
    ...
    Some(SandboxFallbackNotice {
        ...
        reason: reason.clone(),
        risk_summary: "continuing will disable sandbox enforcement; shell access stays degraded, and command hooks, managed code-intel helpers, and stdio MCP servers start degraded until the session re-enables host subprocess surfaces".to_string(),
