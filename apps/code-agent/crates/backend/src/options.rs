...
use crate::ensure_api_key_available;
...
impl AppOptions {
    pub fn from_env_and_args(workspace_root: &Path, env_map: &EnvMap) -> Result<Self> {
        Self::from_env_and_args_iter(workspace_root, env_map, env::args().skip(1))
    }

    pub fn from_env_and_args_iter(
        workspace_root: &Path,
        env_map: &EnvMap,
        ...
        Ok(Self {
            ...
            disabled_tools: workspace_config.disabled_tools,
            plugins,
