...
async fn build_runtime(
    ...
    let skill_catalog = agent_core::skills::load_skill_roots(&skill_roots)
        .await
...
fn build_backend(options: &AppOptions) -> Result<RigModelBackend> {
    let descriptor = RigBackendDescriptor::new(match options.provider {
...
fn print_help() {
    ...
    println!();
    println!("usage:");
    println!("  cargo run --manifest-path apps/code-agent-example/Cargo.toml");
    println!(
        "  cargo run --manifest-path apps/code-agent-example/Cargo.toml -- \"fix the failing test\""
    );
    println!(
